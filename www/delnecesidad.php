...
if ($ids ==-1 ) {
	...
	$stmt = $conn->prepare("DELETE FROM NECESIDAD WHERE (idpaciente=? AND nombre=?)");
	$stmt->bind_param("ss", $ids, $_GET["nombre"]);
	if (!$stmt->execute()) {
    	echo "Execute failed: (" . $stmt->errno . ") " . $stmt->error;
	}else{echo "<head><meta http-equiv=\"refresh\" content=\"0; URL='main.php?token=".$_GET["token"]."'\" /></head>";}
}
